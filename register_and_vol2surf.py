import os
from nipype.interfaces.freesurfer import BBRegister, SampleToSurface
from nipype.interfaces import afni
import nipype.interfaces.freesurfer as fs


# Must have SUBJECTS_DIR set as per standard Freesurfer conventions.
def register_and_vol2surf(mov, subject_id, out_dir, b0=None, feature_name='vol-feature', contrast='t2',
                          sampling_units='frac', sampling_range=(0.2, 0.8, 0.1), sampling_method='average',
                          cleanup=True):
    """
    ...
    feature_name: the name of the feature you would like to project (e.g., FA etc.).
    This is just so the output files are named nicely.
    ...
    sampling range: a float or a tuple of the form: (a float, a float, a float)
    ...
    bbreg = BBRegister(subject_id=subject_id, source_file=b0, subjects_dir=os.environ.get("SUBJECTS_DIR"), init='fsl',
                       contrast_type=contrast, out_reg_file=out_reg_file)
...
def calculate_surface_t1t2_ratio(t2_loc, subject_id, out_dir, t1_loc=None, feature_name='T2', contrast='t2',
                                 sampling_units='frac', sampling_range=(0.2, 0.8, 0.1), sampling_method='average',
                                 cleanup=True):
    """
    ...
    out_reg_file = out_dir + '/' + feature_name + '-T1-reg.dat'
    bbreg = BBRegister(subject_id=subject_id, source_file=t2_loc, init='fsl',
                       subjects_dir=os.environ.get("SUBJECTS_DIR"), contrast_type=contrast, out_reg_file=out_reg_file)
    os.system(bbreg.cmdline)
    # bbregister
    ...
    for hemi in ['lh', 'rh']:
        sampler = SampleToSurface(subject_id=subject_id, hemi=hemi, subjects_dir=os.environ.get("SUBJECTS_DIR"),
                                  ...
                                  sampling_method=sampling_method, sampling_units=sampling_units,
                                  sampling_range=sampling_range, out_file=out_dir + '/' + hemi + '.T1-over-T2.mgz',
                                  out_type='mgz')
        sampler.run()
